...
/// @title EigenFi Pool Interface
/// @notice An interface containing externally accessible functions of the EigenFiPool contract
...
interface IEigenFiPool {
    ...
    ///@notice Emitted when a staker deposits/stakes a supported token into the EigenFi Pool
    ///@param eventId The unique event Id associated with the Deposit event
    ///@param depositor The address of the depositer/staker transfering funds to the EigenFi Pool
    ...
    ///@notice Emitted when a staker withdraws a previously staked tokens from the EigenFi Pool
    ///@param eventId The unique event Id associated with the Withdraw event
    ///@param withdrawer The address of the staker withdrawing funds from the EigenFi Pool
    ...
    ///@notice Emitted when a staker migrates their tokens from the EigenFiPool to Helix.
    ///@param eventId The unique event Id associated with the Migrate event
    ///@param user The address of the staker migrating funds to Helix
    ///@param tokens The addresses of the tokens being being migrated from the EigenFiPool to Helix
    ///@param destination The address which the tokens will be transferred to on Helix
    ///@param migrator The address of the migrator contract which initially receives the migrated tokens
    ///@param amounts The amounts of each token migrated to Helix
    event Migrate(
        uint256 indexed eventId, 
    ...
    ///@notice Stake a specified amount of a particular supported token into the EigenFi Pool
    ///@param _token The token to deposit/stake in the EigenFi Pool
    ///@param _for The user to deposit/stake on behalf of
    ///@param _amount The amount of token to deposit/stake into the EigenFi Pool
    function depositFor(address _token, address _for, uint256 _amount) external;

    ///@notice Stake a specified amount of ether into the EigenFi Pool
    ///@param _for The user to deposit/stake on behalf of
    ///@dev the amount deposited is specified by msg.value
    function depositETHFor(address _for) payable external;

    ///@notice Withdraw a specified amount of a particular supported token previously staked into the EigenFi Pool
    ///@param _token The token to withdraw from the EigenFi Pool
    ///@param _amount The amount of token to withdraw from the EigenFi Pool
    function withdraw(address _token, uint256 _amount) external;

    ///@notice Migrate the staked tokens for the caller from the EigenFi Pool to Helix
    ///@dev called by the staker
    ///@param _tokens The tokens to migrate to Helix from the EigenFi Pool
    ///@param _migratorContract The migrator contract which will initially receive the migrated tokens before moving them to Helix
    ///@param _destination The address which will receive the migrated tokens on Helix
    ///@param _signatureExpiry The timestamp at which the signature in _authorizationSignatureFromHelix expires
    ///@param _authorizationSignatureFromHelix The authorization signature which is signed by the Helix signer and indicates the correct migrator contract
    function migrate(
        address[] calldata _tokens, 
        address _migratorContract, 
        address _destination, 
        uint256 _signatureExpiry, 
        bytes memory _authorizationSignatureFromHelix
    ) external;
    ...
    ///@notice Set/Change the required signer for the migration signature (_authorizationSignatureFromHelix in the migrate() function)
    ///@param _signer The address of the new signer for the migration signature
    ///@dev Only callable by the owner
    function setHelixSigner(address _signer) external;
